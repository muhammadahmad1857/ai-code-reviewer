...
const Logo = ({ slideText = false }: { slideText?: boolean }) => {
  return (
...
text-black-theme transition-all duration-200 group-hover:opacity-0"
      ...
      <span
        className={`text-white-theme font-extrabold hidden sm:block  ${
          slideText
            ? "transition-all  duration-500 transform translate-x-[-20px] opacity-0 group-hover:translate-x-0 group-hover:opacity-100"
            : "opacity-100"
        }`}
      >
        Agentia
